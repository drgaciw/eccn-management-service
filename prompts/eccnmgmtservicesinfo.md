...
- `compareDocumentVersions()`: Compares two versions of a document and generates a diff
...
- `generateDocument()`: Generates a new document from a template
- `linkDocuments()`: Links two documents with a specified relationship type
- `generateDiff()`: Generates a line-by-line diff between two document versions
